...
# Set custom webroot
if [ ! -z "$WEBROOT" ]; then
 sed -i "s#root /srv/www;#root ${WEBROOT};#g" /etc/nginx/sites-available/default.conf
else
 webroot=/srv/www
fi
...
if [ ! -d "/srv/www/.git" ]; then
 ...
 if [ ! -z "$GIT_REPO" ]; then
   ...
   if [ -z "$SKIP_CHOWN" ]; then
     chown -Rf nginx.nginx /srv/www
   fi
 fi
fi

# Enable custom nginx config files if they exist
if [ -f /srv/www/conf/nginx/nginx.conf ]; then
  cp /srv/www/conf/nginx/nginx.conf /etc/nginx/nginx.conf
fi

if [ -f /srv/www/conf/nginx/nginx-site.conf ]; then
  cp /srv/www/conf/nginx/nginx-site.conf /etc/nginx/sites-available/default.conf
fi

if [ -f /srv/www/conf/nginx/nginx-site-ssl.conf ]; then
  cp /srv/www/conf/nginx/nginx-site-ssl.conf /etc/nginx/sites-available/default-ssl.conf
fi
...
if [[ "$RUN_SCRIPTS" == "1" ]] ; then
  if [ -d "/srv/www/scripts/" ]; then
    # make scripts executable incase they aren't
    chmod -Rf 750 /srv/www/scripts/*; sync;
    # run scripts in number order
    for i in `ls /srv/www/scripts/`; do /srv/www/scripts/$i ; done
  else
...
if [ -z "$SKIP_COMPOSER" ]; then
    ...
    if [ -f "/srv/www/composer.lock" ]; then
        if [ "$APPLICATION_ENV" == "development" ]; then
            ...
            composer install --working-dir=/srv/www
        else
            composer global require hirak/prestissimo
            composer install --no-dev --working-dir=/srv/www
        fi
    fi
fi
